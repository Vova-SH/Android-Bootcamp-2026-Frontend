...
# Задачи для Android Frontend разработчика (Kotlin)

## 1. Настройка проекта и архитектура
### 1.1: Создать проект
- Использовать Kotlin в качестве основного языка

### 1.2: Настроить базовые зависимости и инструменты
- Добавить Retrofit2 для сетевых запросов
- Настроить Dagger Hilt
- Реализовать парсинг JSON
- Добавить Coroutines и Flow

### 1.3: Реализовать систему навигации
- Настроить Navigation Component
- Создать граф навигации с основными экранами

## 2. Аутентификация и регистрация
### 2.1: Реализовать экран авторизации
- **UI**: поля email и пароль, кнопка "Войти"
- Валидация вводимых данных
- Интеграция с `POST /api/auth/login`
- Обработка ошибок (неверный пароль, аккаунт не активирован)
- Сохранение JWT токена в `SecureSharedPreferences`

### 2.2: Реализовать экран регистрации
- Форма с полями: email, ФИО, должность, отдел, пароль
- Валидация данных на стороне клиента
- Интеграция с `POST /api/auth/register`
- Обработка подтверждения email

### 2.3: Реализовать экран восстановления пароля
- Ввод email для восстановления
- Отображение состояния отправки инструкций

## 3. Главный экран и навигация
### 3.1: Реализовать главный экран с нижней навигацией
- Вкладки: "Календарь", "Приглашения", "Создать", "Профиль"
- Адаптивный layout для разных размеров экрана

### 3.2: Реализовать боковое меню
- Отображение информации о пользователе
- Кнопка "Выйти"

## 4. Профиль пользователя
### 4.1: Реализовать экран профиля
- Отображение на layout: аватар, ФИО, должность, отдел, email
- Кнопка "Редактировать"
- Интеграция с `GET /api/users/{id}`

### 4.2: Реализовать экран редактирования профиля
- Форма редактирования данных
- Загрузка/изменение аватара
- Валидация изменений
- Интеграция с `PUT /api/users/{id}`

### 4.3: Реализовать экран просмотра профилей других сотрудников
- Отображение информации о коллеге
- Интеграция с `GET /api/users/{id}`

## 5. Календарь и расписание
### 5.1: Реализовать вид дня
- Вертикальная шкала времени с 8:00 до 20:00
- Отображение встреч как цветных блоков в соответствующих слотах
- Возможность скроллинга по времени
- Тап на встрече - переход к деталям
- Интеграция с `GET /api/calendar/day`

### 5.2: Реализовать вид недели
- Сетка: дни по горизонтали, время по вертикали
- Компактное отображение встреч в ячейках
- Горизонтальный скролл по дням
- Интеграция с `GET /api/calendar/week`

### 5.3: Реализовать вид месяца
- Классический месячный календарь
- Отметка дней с встречами
- Тап на дне - переход к виду дня
- Интеграция с `GET /api/calendar/month`

### 5.4: Реализовать переключение между видами календаря
- Сегментированные кнопки для выбора режима
- Сохранение выбранного режима между сессиями

### 5.5: Реализовать быстрый доступ к датам
- Выбор даты из календаря в заголовке
- Свайпы для переключения дней/недель/месяцев
- Кнопка "Сегодня"

## 6. Создание и редактирование встреч
### 6.1: Реализовать экран создания встречи
- Форма с полями: название, описание, место
- Выбор даты и времени (с учетом часовых слотов)
- Добавление участников
- Валидация обязательных полей
- Интеграция с `POST /api/meetings`

### 6.2: Реализовать выбор времени с учетом часовых слотов
- Ограничение выбора строго по часам (9:00, 10:00 и т.д.)
- Автоматическая установка окончания через час
- Возможность изменить продолжительность

### 6.3: Реализовать экран выбора участников
- Список сотрудников с чекбоксами
- Поиск по имени/фамилии (интеграция с `POST /api/users/search`)
- Фильтрация по отделу
- Отображение аватаров и должностей

### 6.4: Реализовать проверку конфликтов расписания
- Отображение предупреждения при выборе занятого времени
- Показ списка занятых участников
- Интеграция с `POST /api/calendar/availability/check`
- Предложение альтернативных слотов

### 6.5: Реализовать экран редактирования встречи
- Форма аналогичная созданию, но с предзаполненными данными
- Интеграция с `PUT /api/meetings/{id}`
- Отправка уведомлений участникам об изменениях

### 6.6: Реализовать экран отмены встречи
- Диалог подтверждения с возможностью указать причину
- Интеграция с `DELETE /api/meetings/{id}`

## 7. Управление приглашениями
### 7.1: Реализовать экран входящих приглашений
- Список приглашений с карточками
- Фильтрация по статусу (новые, принятые, отклоненные, просроченные)
- Отображение: название, время, организатор, статус
- Интеграция с `GET /api/invitations/incoming`

### 7.2: Реализовать экран деталей приглашения
- Полная информация о встрече
- Кнопки "Принять" и "Отклонить"
- Интеграция с `POST /api/invitations/{id}/accept` и `/decline`

### 7.3: Реализовать экран исходящих приглашений
- Список созданных пользователем встреч
- Отображение статусов участников (принял/отклонил/ожидает)
- Возможность отправить напоминание
- Интеграция с `GET /api/invitations/outgoing`

## 8. Детали встречи
### 8.1: Реализовать экран детальной информации о встрече
- Отображение на layout: название, описание, место, время
- Список участников с их статусами
- Для организатора: кнопки редактирования/отмены
- Возможность просмотреть профиль участника

### 8.2: Реализовать просмотр статусов участников
- Визуализация статусов (цветовые индикаторы)
- Статистика по ответам
- Возможность фильтровать по статусу

## 9. Уведомления и напоминания
### 9.1: Интегрировать Firebase Cloud Messaging (FCM)
- Регистрация устройства для получения push-уведомлений
- Обработка входящих уведомлений
- Навигация к соответствующему экрану при тапе на уведомление

### 9.2: Реализовать локальные уведомления для напоминаний
- Создание напоминаний о встречах за 15/30/60 минут

### 9.3: Реализовать экран настроек уведомлений
- Включение/выключение типов уведомлений
- Настройка звука и вибрации
- Установка времени напоминаний
- Интеграция с `PUT /api/users/notifications/settings`

## 10. Поиск и фильтрация
### 10.1: Реализовать поиск встреч
- Поисковая строка в календаре и списке приглашений
- Поиск по названию встречи и участникам
- Интеграция с соответствующими бэкенд-эндпоинтами

### 10.2: Реализовать фильтрацию встреч
- Фильтры по статусу (предстоящие, прошедшие, отмененные)
- Фильтры по типу участия (обязательные/опциональные)
- Сохранение выбранных фильтров

## 11. Дополнительные функции
### 11.1: Реализовать смену темы оформления
- Поддержка светлой и темной темы
- Автоматическое переключение по системным настройкам
- Кастомизация цветовых акцентов

### 11.2: Реализовать блокировку времени
- Возможность отметить время как недоступное
- Указание причины блокировки
- Интеграция с `POST /api/calendar/block`

### 11.3: Реализовать быстрые действия
- Длинный тап на свободном слоте - быстрое создание встречи
- Перетаскивание встречи для изменения времени
- Дублирование существующей встречи

### 11.4: Реализовать повторяющиеся встречи
